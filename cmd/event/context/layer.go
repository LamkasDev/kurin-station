...
func ProcessKurinEventLayerContext(manager *event.KurinEventManager, layer *event.KurinEventLayer, game *gameplay.KurinGame) *error {
	...
	if data.Position != nil {
		data.HoveredItem = -1
		if manager.Renderer.WindowContext.MousePosition.X >= data.Position.X && manager.Renderer.WindowContext.MousePosition.X <= data.Position.X + context.KurinRendererLayerContextDataItemWidth {
			hovered := int(math.Floor((float64(manager.Renderer.WindowContext.MousePosition.Y)-float64(data.Position.Y))/context.KurinRendererLayerContextDataItemHeight))
	...
	if manager.Mouse.PendingRight != nil {
		...
		if tile != nil {
			position := manager.Renderer.WindowContext.MousePosition
